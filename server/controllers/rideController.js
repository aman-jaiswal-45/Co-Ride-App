...
exports.createRide = async (req, res) => {
    try {
        ...
        // Instead of checking for a 'Driver' role, we check for vehicle details.
        ...
        const minDepartureTime = new Date(Date.now() + 5 * 60 * 1000);
        if (new Date(req.body.departureTime) < minDepartureTime) {
...
exports.manageBookingRequest = async (req, res) => {
    ...
    try {
        ...
        console.error('Error in manageBookingRequest:', error);
...
exports.getRide = async (req, res) => {
    try {
        const ride = await Ride.findById(req.params.id).populate('driver', 'name vehicleDetails');
        if (!ride) return res.status(404).json({ success: false, error: 'Ride not found' });
        res.status(200).json({ success: true, data: ride });
    } catch (error) {
        res.status(500).json({ success: false, error: 'Server Error' });
